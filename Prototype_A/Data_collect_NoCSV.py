...
def Take(iso, s_speed, imu, date, gps):
    ...
    while True :
        ...
        hdr = Header({'SIMPLE': True, 'XTENSION' : 'IMAGE','BITPIX':8, 'NAXIS':3, 'NAXIS1':1472,
                      'NAXIS2': 1472, 'CTYPE3':'RGB', 'TIME':time_gps, 'LAT':lat , 'LONG':long,
                      'ALT':h/100, 'q_x': quat[0], 'q_y':quat[1], 'q_z' : quat[2], 'q_w' : quat[3],
        ...
        if GPIO.input(26) == True :
            camera.close()
            os.system('pkill $(pgrep -f /home/pi/Projet_Stage/Prototype_A/ST/Data_collect_NoCSV.py)')
            break
